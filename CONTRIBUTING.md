...
- Headless first: every workflow should be scriptable before it needs a TUI.
...
another installed `hz`. The dev zsh rc file also loads `hz shell zsh`, so
auto-cd and completion behavior use the local binary by default.
