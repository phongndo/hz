use super::{Strategy, StrategyInit, create_private_directory, is_workspace_marker};
...
impl Strategy for LinuxReflinkStrategy {
    fn copy_directory(
        ...
        match mode {
            CopyMode::All => clone_directory_linux(from, to, workspace_id),
            CopyMode::Filtered => clone_directory_linux_filtered(from, to, workspace_id),
        }
    }
...
pub(super) fn clone_directory_linux(from: &Path, to: &Path, workspace_id: &str) -> Result<()> {
